...
export class HomeServerManager implements HomeServerManagerInterface {
  ...
  async setHomeServerConfiguration(configurationJSONString: string): Promise<void> {
    try {
      if (!this.homeServerDataLocation) {
        throw new Error(c('B6.Preferences.HomeServer.Error').t`Home server data location is not set.`)
      }
      ...
      const configurationFile: HomeServerConfigurationFile = {
        ...
        info: {
          ...
          information: c('B6.Preferences.HomeServer.Info')
            .t`The values below are encrypted with a key created by the desktop application after installation. The key is stored in your secure device keychain.`,
          instructions: c('B6.Preferences.HomeServer.Info')
            .t`Put this file inside your home server data location to restore your home server configuration.`,
        },
