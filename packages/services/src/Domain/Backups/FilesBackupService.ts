...
import { NoteType, AppName, jtString } from '@standardnotes/features'
...
const PlaintextBackupsDirectoryName = c('B6.Preferences.Backups.Label').t`Plaintext Backups`
export const TextBackupsDirectoryName = c('B6.Preferences.Backups.Label').t`Text Backups`
export const FileBackupsDirectoryName = c('B6.Preferences.Backups.Label').t`File Backups`
  ...
  implements BackupServiceInterface, InternalEventHandlerInterface
{
  ...
  private async performBackupOperation(file: FileItem): Promise<'success' | 'failed' | 'aborted'> {
    ...
    const metaFile: FileBackupMetadataFile = {
      info: {
        warning: c('B6.Preferences.Backups.Info').t`Do not edit this file.`,
        information: c('B6.Preferences.Backups.Info')
          .t`The file and key data below is encrypted with your account password.`,
        instructions: jtString(
          c('B6.Preferences.Backups.Info')
            .jt`Drag and drop this metadata file into the File Backups preferences pane in the ${AppName} desktop or web application interface.`,
        ),
      },
