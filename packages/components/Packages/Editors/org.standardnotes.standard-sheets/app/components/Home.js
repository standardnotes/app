      ...
      function () {
        $('#spreadsheet').kendoSpreadsheet({
          ...
          excelImport: (event) => {
            ...
            if (
              !confirm(
                'Note that importing from Excel may cause very large file sizes within Standard Notes, which may affect performance. You may continue with import, but if you notice performance issues, it is recommended you manually import data instead.',
              )
            ) {
              event.preventDefault()
              return
  ...
  connectToBridge() {
    this.componentRelay = new ComponentRelay({
      ...
      handleRequestForContentHeight: () => {
        let height = 0
        height += document.querySelector('.k-spreadsheet-tabstrip')?.offsetHeight ?? 0
        height += document.querySelector('.k-spreadsheet-action-bar')?.offsetHeight ?? 0
        height += document.querySelector('.k-spreadsheet-view')?.offsetHeight ?? 0
        height += document.querySelector('.k-spreadsheet-sheets-bar')?.offsetHeight ?? 0
        height += document.querySelector('.k-spreadsheet-view-size')?.offsetHeight ?? 0
        return height
      },
    })
