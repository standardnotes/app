...
import { componentsCdn, version } from '../../package.json'
...
const MobilePackageTagName = '@standardnotes/mobile'
...
export class ComponentManager extends SNComponentManager {
  ...
  private cdnUrlForFeature(identifier: FeatureIdentifier): string {
    ...
    const tagPath = `${MobilePackageTagName}@${appVersion}`.replaceAll('@', '%40')
