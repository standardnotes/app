      ...
      <div className="sn-component">
        ...
        >
          <div className="flex flex-shrink-0 items-center justify-between border-b border-solid border-border bg-contrast px-6 py-3 text-text">
            <div className="text-base font-medium">{c('B5.Sync.Title').t`Out of Sync`}</div>
            <a onClick={this.close} className="text-sm font-bold text-info">
              {c('B5.Sync.Action').t`Close`}
            </a>
          </div>
          <div className="h-full flex-grow overflow-scroll overflow-x-auto overflow-y-auto p-6 text-sm leading-tight">
            <div className="flex flex-col">
              <div className="mb-3.5">
                {c('B5.Sync.Info')
                  .t`We've detected that the data in the current application session may not match the data on the server. An attempt was made to auto-resolve the issue, but it was unable to reconcile the differences.`}
              </div>
              <div className="sk-p sk-panel-row mb-3.5">
                <div className="sk-panel-column">
                  <div className="mb-2 font-bold">{c('B5.Sync.Subtitle').t`Option 1 — Restart Application:`}</div>
                  <div>
                    {c('B5.Sync.Info').t`Quit the application and re-open it. Sometimes, this may resolve the issue.`}
                  </div>
                </div>
              </div>
              <div className="sk-p sk-panel-row">
                <div className="sk-panel-column">
                  <div className="mb-2 font-bold">{c('B5.Sync.Subtitle').t`Option 2 — Sign Out and Back In:`}</div>
                  <div>
                    {c('B5.Sync.Info')
