...
const NoAccountWarningContent = ({ accountMenuController, noAccountWarningController }: Props) => {
  ...
  return (
    <div className="mt-4 grid grid-cols-1 rounded-md border border-border p-4">
      ...
      >
        {c('B1.Account.Session.Action').t`Open Account menu`}
      </Button>
      <button
        onClick={hideWarning}
        title={c('B1.Account.Session.Label').t`Ignore warning`}
        aria-label={c('B1.Account.Session.Label').t`Ignore warning`}
        style={{ height: '20px' }}
