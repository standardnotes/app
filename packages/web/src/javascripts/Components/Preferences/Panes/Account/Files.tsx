...
const FilesSection: FunctionComponent<Props> = ({ application }) => {
  ...
  return (
    <PreferencesGroup>
      <PreferencesSegment>
        <Title>{c('B7.Subscription.Info').t`Files`}</Title>
        <Subtitle>{c('B7.Subscription.Info').t`Storage Quota`}</Subtitle>
        {isLoading ? (
          <div className="mt-2">
          ...
          <>
            <div className="mb-1 mt-1">
              ...
              {c('B7.Subscription.Info').t`of`}{' '}
              <span>
                {application.sessions.isSignedIntoFirstPartyServer()
                  ? formatSizeToReadableString(filesQuotaTotal)
                  : '∞'}
              </span>{' '}
              {c('B7.Subscription.Info').t`used`}
            </div>
            <progress
              className="progress-bar w-full"
              aria-label={c('B7.Subscription.Label').t`Files storage used`}
