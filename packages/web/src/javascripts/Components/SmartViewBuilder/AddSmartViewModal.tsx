...
const AddSmartViewModal = ({ controller, platform }: Props) => {
    ...
    (): ModalAction[] => [
      ...
      },
      {
        label: isSaving ? (
          ...
          c('B3.Notes.TagsLinkedItems.Action').t`Save`
        ) : (
          c('B3.Notes.TagsLinkedItems.Action').t`Validate`
        ),
        onClick: save,
        disabled: isSaving,
        mobileSlot: 'right',
  ...
  return (
    <Modal title={c('B3.Notes.TagsLinkedItems.Action').t`Add Smart View`} close={closeModal} actions={modalActions}>
      <div className="px-4 py-4">
        <div className="flex h-full flex-col gap-4">
          <div className="flex items-center gap-2.5">
            <div className="text-sm font-semibold">{c('B3.Notes.TagsLinkedItems.Label').t`Title:`}</div>
            <input
          ...
          <div className="flex items-center gap-2.5">
            <div className="text-sm font-semibold">{c('B3.Notes.TagsLinkedItems.Label').t`Icon:`}</div>
            <button
              className="rounded border border-border p-2"
              aria-label={c('B3.Notes.TagsLinkedItems.Action').t`Change icon`}
            ...
            <Popover
              title={c('B3.Notes.TagsLinkedItems.Label').t`Choose icon`}
          ...
          <div className="flex flex-grow flex-col gap-2.5">
            <div className="text-sm font-semibold">{c('B3.Notes.TagsLinkedItems.Label').t`Predicate:`}</div>
            <TabsContainer
              className="flex flex-grow flex-col"
              tabs={[
                {
                  id: 'builder',
                  title: c('B3.Notes.TagsLinkedItems.Label').t`Builder`,
                },
                {
                  id: 'custom',
                  title: c('B3.Notes.TagsLinkedItems.Label').t`Custom (JSON)`,
                },
              ]}
              state={tabState}
            >
              ...
              <TabPanel state={tabState} id="custom" className="flex flex-grow flex-col">
                ...
                {customPredicateJson && isCustomJsonValidPredicate === false && (
                  <div className="border-t border-border px-2.5 py-1.5 text-sm text-danger">
                    {c('B3.Notes.TagsLinkedItems.Error').t`Invalid JSON. Double check your entry and try again.`}
                  </div>
                )}
              </TabPanel>
            </TabsContainer>
            {tabState.activeTab === 'custom' && (
              <div className="flex flex-col gap-1.5 rounded-md border-2 border-info-backdrop bg-info-backdrop px-4 py-3">
                ...
                >
                  <div className="text-sm font-semibold">{c('B3.Notes.TagsLinkedItems.Label').t`Examples`}</div>
                ...
                >
                  <div className="text-sm font-medium">
                    {c('B3.Notes.TagsLinkedItems.Info').t`1. List notes that are conflicted copies of another note:`}
                  </div>
                  <CopyableCodeBlock code={ConflictedNotesExampleCode} />
                  <div className="text-sm font-medium">
                    {c('B3.Notes.TagsLinkedItems.Label')
