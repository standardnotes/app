  ...
  return (
    <div className="grid grid-cols-1 rounded-md border border-border p-4">
      <div className="flex items-center">
        ...
        <h1 className="sk-h3 m-0 text-sm font-semibold">{c('B1.Account.Session.Title').t`Save your recovery code`}</h1>
      </div>
      <p className="col-start-1 col-end-3 m-0 mt-1 text-sm">
        {c('B1.Account.Session.Info')
          .t`Your recovery code allows you access to your account in the event you lose your 2FA authenticating device or app. Save your recovery code in a safe place outside your account.`}
      </p>
      {errorMessage && <div>{errorMessage}</div>}
      {!recoveryCode && (
        <Button primary small className="col-start-1 col-end-3 mt-3 justify-self-start uppercase" onClick={onClickShow}>
          {c('B1.Account.Session.Action').t`Show Recovery Code`}
        </Button>
      )}
      {recoveryCode && (
        <div className="group relative mt-2 rounded border border-border px-3 py-2 text-sm font-semibold">
          <StyledTooltip label={c('B1.Account.Session.Label').t`Copy to clipboard`} className="!z-modal">
            <button
